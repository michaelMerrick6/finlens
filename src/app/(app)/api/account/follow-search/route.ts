...
import { searchFundSuggestions, searchInsiderSuggestions, searchPoliticianSuggestions } from '@/lib/account-server';
...
function parseActorType(value: string | null | undefined): ActorType {
  ...
  if (normalized === 'politician' || normalized === 'insider' || normalized === 'fund') {
    return normalized;
  }
  ...
  try {
    ...
    const suggestions =
      actorType === 'politician'
        ...
        : actorType === 'fund'
          ? await searchFundSuggestions(query)
