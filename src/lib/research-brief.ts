import { type ScreenResult } from './research-screen';
...
export function buildResearchBrief(result:ScreenResult):ResearchBrief {
 ...
 const maximum=Math.max(...stocks.map(s=>s.politicians));
 const leaders=stocks.filter(s=>s.politicians===maximum);
 const first=stocks[0];
 const name=(s:typeof first)=>s.company?.company_name||s.trades[0]?.asset_name||s.ticker;
 const title=stocks.length===1?`${name(first)}: the disclosed activity.`:leaders.length===1?`${name(first)} leads by distinct ${activity}.`:`${leaders.length} companies share the lead by distinct ${activity}.`;
 const lead=stocks.length===1?`${first.politicians} distinct ${first.politicians===1?'politician appears':'politicians appear'} in ${first.trades.length} reported ${action} for ${first.ticker} in this screen.`:
 ...
 if(index===0&&stocks.length>1){const next=stocks[1];paragraphs.push(s.politicians===next.politicians?`${s.ticker} is tied on politician count. Transaction count determines display order within a tie, followed by ticker; it does not make this company a unique leader.`:`For comparison, ${next.ticker} appears for ${next.politicians} distinct ${activity} across ${next.trades.length} transactions in the same screen.`);}
 ...
 return {ticker:s.ticker,name:name(s),headline:stocks.length===1?'What the filings show.':s.politicians===maximum?(leaders.length>1?'A shared lead in reported activity.':filters.activity==='buy'?'The widest buying interest in this window.':filters.activity==='sell'?'The widest selling activity in this window.':'The widest participation in this window.'):buys&&sells?'Activity on both sides of the trade.':s.trades.length>first.trades.length?`More transactions, fewer distinct ${activity}.`:s.trades.length>s.politicians?`Repeat ${action} within a smaller group.`:'A narrower footprint in the filings.',paragraphs,members,dates:dates.length?`Reported transaction dates: ${dates[0]}${dates.length>1?` to ${dates.at(-1)}`:''}.${missingDates?` ${missingDates} records have no transaction date.`:''}`:'Transaction dates are unavailable.'};
 })};
}
