...
const RANGE_LABELS: Record<CongressClusterRange, string> = {
  week: 'disclosed in the last 7 days',
  month: 'disclosed in the last 30 days',
  ytd: 'disclosed year to date',
};

function minimumLabel(value: number) {
  return value > 0 ? `${formatCompactCurrency(value)}+` : 'Not reported';
}
      ...
      {Array.from({ length: 4 }).map((_, index) => (
        <div key={index} className="flex animate-pulse items-center gap-3 rounded-xl bg-white/[0.018] p-3">
          ...
          <div className="min-w-0 flex-1 space-y-2">
            ...
            <div className="h-2.5 w-52 max-w-full rounded-full bg-white/[0.04]" />
          </div>
...
export default function CongressBuyingTransactionsModal({
  company,
  range,
  data,
  loading,
  ...
  company: CongressBuyingCompany | null;
  range: CongressClusterRange;
  data: CongressBuyingTransactionsData | null;
  loading: boolean;
  error: string;
  onClose: () => void;
}) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[200] flex items-end justify-center bg-black/70 p-2 backdrop-blur-sm sm:items-center sm:p-6">
      ...
      <section
        role="dialog"
        aria-modal="true"
        aria-labelledby="congress-transactions-title"
      ...
      >
        <header className="border-b border-white/[0.07] px-4 pb-4 pt-4 sm:px-5 sm:pt-5">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <div className="text-[10px] font-semibold uppercase tracking-[0.17em] text-emerald-300/75">
                Congressional buys · {RANGE_LABELS[range]}
              </div>
              <h2 id="congress-transactions-title" className="mt-1.5 text-xl font-semibold tracking-tight text-white">
            ...
            <button
              ...
              aria-label="Close"
              onClick={onClose}
              className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.025] text-zinc-500 transition hover:bg-white/[0.06] hover:text-white"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <div className="mt-4 grid grid-cols-3 divide-x divide-white/[0.06] overflow-hidden rounded-xl border border-white/[0.07] bg-white/[0.018]">
            ...
            <div className="px-3 py-2.5">
              <div className="text-sm font-semibold tabular-nums text-white">{company.actorCount.toLocaleString()}</div>
              <div className="mt-0.5 text-[9px] uppercase tracking-[0.1em] text-zinc-600">Lawmakers</div>
            </div>
            <div className="px-3 py-2.5">
              <div className="text-sm font-semibold tabular-nums text-white">{company.tradeCount.toLocaleString()}</div>
              <div className="mt-0.5 text-[9px] uppercase tracking-[0.1em] text-zinc-600">Buys</div>
            </div>
          </div>
        </header>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          {loading ? (
            ...
            <div className="px-5 py-12 text-center text-sm text-zinc-500">No matching transactions were found.</div>
          ) : (
              ...
              {data.transactions.map((transaction) => (
                <article key={transaction.id} className="flex items-start gap-3 px-4 py-3.5 sm:px-5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 flex flex-wrap items-center gap-x-1.5 text-[11px] text-zinc-500">
                      {transaction.chamber ? <span>{transaction.chamber}</span> : null}
                      {transaction.chamber ? <span className="text-white/15">·</span> : null}
                      <span>
                        Traded {transaction.transactionDate ? formatCalendarDate(transaction.transactionDate, 'UTC') : 'date not reported'}
                      </span>
                    </div>
                    <div className="mt-1 truncate text-[11px] text-zinc-700">
                      Disclosed {formatCalendarDate(transaction.publishedDate, 'UTC')}
                  ...
                  <div className="shrink-0 text-right">
                    <div className="text-sm font-semibold tabular-nums text-white">
                      {transaction.amountRange || 'Not reported'}
                    </div>
                    {transaction.sourceUrl ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="mt-1 inline-flex items-center gap-1 text-[10px] text-zinc-600 transition hover:text-zinc-300"
                      >
        ...
        <footer className="flex items-center justify-between gap-4 border-t border-white/[0.07] px-4 py-3 sm:px-5">
          ...
          <Link
            ...
            className="inline-flex shrink-0 items-center gap-1 text-xs font-medium text-zinc-400 transition hover:text-white"
          >
            Stock page <ArrowUpRight className="h-3.5 w-3.5" />
          </Link>
        </footer>
      </section>
    </div>,
