...
function HqContent(){
 ...
 useEffect(()=>{
  ...
  fetch('/api/hq',{headers:{Authorization:`Bearer ${session.access_token}`, "x-hq-password":submittedPassword},cache:'no-store',signal:controller.signal}).then(async r=>{const d=await r.json();if(!r.ok)throw Error(d.error);return d;}).then(d=>{if(!controller.signal.aborted)setData(d);}).catch(e=>{if(!controller.signal.aborted)setError(e.message);}).finally(()=>{if(!controller.signal.aborted)setBusy(false);});
  return()=>controller.abort();
 },[session,revision,submittedPassword]);
 return <>
 ...
 {!data && <form className="hq-stat" style={{maxWidth:420}} onSubmit={e=>{e.preventDefault();setSubmittedPassword(password);setPassword("");setRevision(n=>n+1);}}>
