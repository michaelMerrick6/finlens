...
export function AnalysisPage(){
 ...
 const [data,setData]=useState<Data|null>(null),[error,setError]=useState(''),[busy,setBusy]=useState(true),[selected,setSelected]=useState<AnalysisTrade|null>(null),[limit,setLimit]=useState(25);
 ...
 {data&&<><p className="analysis-period">{dateLabel(data.start)} – {dateLabel(data.end)} · {stocks.length} stocks</p><details className="analysis-method"><summary>How these numbers work</summary><p>Ranges sum reported ranges and estimated standard ranges where only a recognized lower bound was retained. These reconstructed upper bounds are assumptions, not source-verified amounts. Unrecognized or open-ended amounts are excluded from dollar totals and counted below each stock. Largest purchases ranks by the lower bound. Net estimates purchases minus sales using range midpoints; it shows Incomplete if any amounts are missing. Stocks and options are separate. Known contributions, unidentified tickers and unsupported asset types are excluded. Duplicate source row IDs are counted once; separate amended filings may still require review. Buyer and seller counts cover the selected period; the same politician can appear in both groups.</p><p>{data.scanned} records scanned; {data.excluded} excluded by these rules.</p></details>
 {!stocks.length&&<div className="empty-state">No eligible activity in this period.</div>}
 <div className="analysis-list">{stocks.slice(0,limit).map((s,i)=><details className="analysis-stock" key={`${period}-${basis}-${instrument}-${s.ticker}`}><summary><span className="analysis-identity"><span className="muted">{i+1}</span><CompanyLogo ticker={s.ticker}/><strong>{s.ticker}</strong></span><span><b>{s.buyers}</b> buyers · <b>{s.sellers}</b> sellers</span><span><small>Purchases</small>{range(s.purchaseMin,s.purchaseMax)}</span><span><small>Sales</small>{range(s.saleMin,s.saleMax)}</span><NetAmount stock={s}/><span>View activity ↓</span></summary><div className="analysis-evidence"><p>Latest disclosure: {dateLabel(s.latest)} · {s.trades.length} recorded transactions{s.unknownAmounts?` · ${s.unknownAmounts} amounts excluded from dollar totals`:''}{s.inferredAmounts?` · ${s.inferredAmounts} standard ranges estimated from stored lower bounds`:''}</p><div className="analysis-trade-heading" aria-hidden="true"><span>Politician</span><span>Activity</span><span>Disclosed amount</span><span>Transaction date</span><span>Source</span></div>{s.trades.map(t=><button key={t.id} className="analysis-trade" onClick={()=>setSelected(t)}><span>{t.politician_name}</span><span className={`analysis-direction ${t.transaction_type === "buy" ? "purchase" : "sale"}`}>{t.transaction_type==='buy'?'Purchase':'Sale'}</span><span>{t.amount_range||'Amount unavailable'}</span><span>Traded {dateLabel(t.transaction_date)}</span><span>View filing details →</span></button>)}</div></details>)}</div>
 {stocks.length>limit&&<div className="load-more"><button className="button secondary" onClick={()=>setLimit(n=>n+25)}>Show more stocks</button></div>}</>}
