...
- Discovery inventories all House PTR indexes from 2012 and exhausts the Senate feed in calendar-year windows from the same date. Duplicate page boundaries trigger smaller, disjoint date windows instead of silently dropping duplicates. Set `--start-year` to change this explicit boundary. Missing/malformed indexes, duplicate keys, changing Senate totals, and truncated pagination are errors.
