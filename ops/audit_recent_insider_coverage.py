...
def load_recent_insider_rows(supabase, *, days: int) -> list[dict]:
    ...
    offset = 0
    while True:
        # created_at is indexed. The audit only needs rows from recent capture
        # runs, and scanning published_date causes timeouts as the table grows.
        response = (
            supabase.table("insider_trades")
            .select("ticker, filer_name, filer_relation, transaction_date, published_date, transaction_code, amount, price, value, source_url, created_at")
