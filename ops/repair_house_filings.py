...
def execute_with_retry(builder, *, attempts: int = 5, base_delay: float = 2.0):
    last_exc = None
...
def replace_house_doc(year: int, doc_id: str, filing: dict, members_db: list[dict], company_lookup: list[dict]) -> tuple[int, int]:
    ...
    execute_with_retry(supabase.table("politician_trades").delete().ilike("doc_id", f"{prefix}%"))

    inserted = 0
    for index in range(0, len(prepared_trades), 50):
        chunk = prepared_trades[index : index + 50]
        execute_with_retry(supabase.table("politician_trades").insert(chunk))
        inserted += len(chunk)
