...
def ensure_watchlist_tickers(supabase, watchlist_id: str, tickers: Iterable[str], *, alert_mode: str) -> int:
    payload = [{"watchlist_id": watchlist_id, "ticker": ticker.upper(), "alert_mode": alert_mode} for ticker in tickers]
    if not payload:
        return 0
    try:
        supabase.table("watchlist_tickers").upsert(payload, on_conflict="watchlist_id,ticker").execute()
    except Exception:
        fallback = [{"watchlist_id": row["watchlist_id"], "ticker": row["ticker"]} for row in payload]
        supabase.table("watchlist_tickers").upsert(fallback, on_conflict="watchlist_id,ticker").execute()
    return len(payload)


def ensure_watchlist_actors(supabase, watchlist_id: str, actors: Iterable[dict], *, default_alert_mode: str) -> int:
    payload = [
        {
            ...
            "alert_mode": actor.get("alert_mode") or default_alert_mode,
    ...
    try:
        ...
        supabase.table("watchlist_actors").upsert(fallback, on_conflict="watchlist_id,actor_type,actor_key").execute()
    return len(payload)
...
def build_politician_targets(values: Iterable[str], members: list[dict], *, alert_mode: str) -> list[dict]:
    payload: list[dict] = []
...
def build_named_actor_targets(actor_type: str, values: Iterable[str], *, alert_mode: str) -> list[dict]:
    payload: list[dict] = []
    seen_keys: set[tuple[str, str]] = set()
    for value in values:
        actor_key = normalize_actor_key(value)
...
def main():
    ...
    parser.add_argument(
        "--ticker-mode",
        default="unusual",
        choices=["activity", "unusual", "both"],
        help="Alert mode for ticker follows.",
    )
    ...
    parser.add_argument(
        ...
        default="unusual",
        choices=["activity", "unusual", "both"],
        help="Alert mode for politician follows.",
    )
    ...
    parser.add_argument(
        ...
        default="unusual",
        choices=["activity", "unusual", "both"],
        help="Alert mode for insider follows.",
    )
    ...
    parser.add_argument(
        ...
        default="unusual",
        choices=["activity", "unusual", "both"],
        help="Alert mode for fund follows.",
    )
    args = parser.parse_args()

    supabase = get_supabase_client()
    event_types = parse_csv(args.event_types)
    destination = normalize_destination(args.channel, args.destination)
    ...
    if args.global_subscription:
        ...
        print({"mode": "global", "channel": args.channel, "destination": destination, "subscription_id": subscription_id})
        return
    ...
    actor_targets = []
    actor_targets.extend(build_politician_targets(politician_values, members, alert_mode=args.politician_mode))
    actor_targets.extend(build_named_actor_targets("insider", insider_values, alert_mode=args.insider_mode))
    actor_targets.extend(build_named_actor_targets("fund", fund_values, alert_mode=args.fund_mode))

    watchlist_id = ensure_watchlist(supabase, args.owner_key, args.watchlist_name)
    watchlist_ticker_count = ensure_watchlist_tickers(supabase, watchlist_id, tickers, alert_mode=args.ticker_mode)
    watchlist_actor_count = ensure_watchlist_actors(supabase, watchlist_id, actor_targets, default_alert_mode="unusual")
    subscription_id = ensure_subscription(
        supabase,
        channel=args.channel,
        watchlist_id=watchlist_id,
    ...
    print(
        {
            ...
            "ticker_mode": args.ticker_mode,
            "politician_mode": args.politician_mode,
