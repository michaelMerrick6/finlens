...
CREATE INDEX IF NOT EXISTS idx_congress_trade_filing_prefix_c ON public.politician_trades((lower(doc_id) COLLATE "C"));
CREATE INDEX IF NOT EXISTS idx_congress_raw_filing_prefix_c ON public.raw_filings((lower(source_document_id) COLLATE "C")) WHERE source='congress';
CREATE INDEX IF NOT EXISTS idx_congress_signal_filing_prefix_c ON public.signal_events((lower(source_document_id) COLLATE "C")) WHERE source='congress' AND signal_type='politician_trade';
...
BEGIN
    ...
    END IF;
    SELECT count(*) INTO prior_count FROM politician_trades WHERE lower(doc_id) COLLATE "C" >= target_filing_id || '-' AND lower(doc_id) COLLATE "C" < target_filing_id || '.';
    SELECT EXISTS (
        (SELECT to_jsonb(t) - 'id' - 'created_at' FROM politician_trades t WHERE lower(doc_id) COLLATE "C" >= target_filing_id || '-' AND lower(doc_id) COLLATE "C" < target_filing_id || '.'
         EXCEPT SELECT value FROM jsonb_array_elements(trades))
        UNION ALL
        (SELECT value FROM jsonb_array_elements(trades)
         EXCEPT SELECT to_jsonb(t) - 'id' - 'created_at' FROM politician_trades t WHERE lower(doc_id) COLLATE "C" >= target_filing_id || '-' AND lower(doc_id) COLLATE "C" < target_filing_id || '.')
    ) INTO changed;
    ...
    IF changed THEN
        ...
        WHERE source='congress' AND signal_type='politician_trade'
          AND lower(source_document_id) COLLATE "C" >= target_filing_id || '-' AND lower(source_document_id) COLLATE "C" < target_filing_id || '.';
        ...
        snapshot := jsonb_build_object(
            'trades', (SELECT coalesce(jsonb_agg(to_jsonb(t)), '[]') FROM politician_trades t WHERE lower(doc_id) COLLATE "C" >= target_filing_id || '-' AND lower(doc_id) COLLATE "C" < target_filing_id || '.'),
            'signals', (SELECT coalesce(jsonb_agg(to_jsonb(e)), '[]') FROM signal_events e WHERE id=ANY(removed_ids || changed_ids)),
            'raw_filings', (SELECT coalesce(jsonb_agg(to_jsonb(r)), '[]') FROM raw_filings r WHERE source='congress' AND lower(source_document_id) COLLATE "C" >= target_filing_id || '-' AND lower(source_document_id) COLLATE "C" < target_filing_id || '.'),
    ...
    DELETE FROM politician_trades WHERE lower(doc_id) COLLATE "C" >= target_filing_id || '-' AND lower(doc_id) COLLATE "C" < target_filing_id || '.' AND NOT (doc_id=ANY(trade_keys));
    ...
    DELETE FROM raw_filings WHERE source='congress' AND lower(source_document_id) COLLATE "C" >= target_filing_id || '-' AND lower(source_document_id) COLLATE "C" < target_filing_id || '.'
        AND NOT (source_document_id=ANY(trade_keys));
